...
public abstract record Truck(string TruckType, Size Size, int Age, Location Location) : ITruck
{
    ...
    private int ConsumptionCorrectionForAge => Age / 3;

    private string FormattedAge => Age switch
    {
        0 => "-new-",
        1 => "1 year",
        _ => $"{Age} years",
    ...
    public abstract int MaxPayloadInTons { get; }
    ...
    protected virtual double CalculatePriceFactor()
    {
        ...
        var priceFactor  = 1 + randomFactor / 100.0 - Age * 0.03;
        return priceFactor;
    }
}